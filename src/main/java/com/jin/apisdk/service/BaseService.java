...
import com.jin.apisdk.exception.ErrorCode;

import com.jin.apisdk.exception.ErrorResponse;
import com.jin.apisdk.model.request.BaseRequest;
import com.jin.apisdk.model.response.ResultResponse;
import com.jin.apisdk.utils.SignUtils;
import lombok.Data;
...
//    private String gatewayHost = "http://47.95.193.198:8090/api";
    ...
    private <O, T extends ResultResponse> HttpRequest getHttpRequestByRequestMethod(BaseRequest<O, T> request) throws ApiException {
        ...
        if (path.startsWith(gatewayHost)) {  // 以gateway开头 则 真实的path应 去除掉gateway的path
    ...
    private <O, T extends ResultResponse> String splicingGetRequest(BaseRequest<O, T> request, String path) {
        ...
        if (!request.getRequestParams().isEmpty()) {   // 拼接上get请求的参数 得到实际的请求路径(带参数的)
            urlBuilder.append("?");
